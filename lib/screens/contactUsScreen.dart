...
class _ContactUsScreenState extends State<ContactUsScreen> {
  ...
  Future<void> _submitContact() async {
    ...
    try {
      final url = Uri.parse(
        "https://churppy.eurekawebsolutions.com/api/customer_save_contact.php",
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: isDesktop
            ? _buildDesktopLayout(fs, hp, wp)
            : _buildMobileTabletLayout(fs, hp, wp, isTablet),
      ),
    );
  }
  ...
  Widget _buildMobileTabletLayout(double Function(double) fs, double Function(double) hp, double Function(double) wp, bool isTablet) {
    return Column(
      children: [
        ...
        Expanded(
          child: SingleChildScrollView(
            padding: EdgeInsets.symmetric(horizontal: isTablet ? wp(8) : wp(5), vertical: hp(2)),
            child: Column(
              children: [
                if (!isTablet) ...[
  ...
  Widget _buildBackButton(double Function(double) fs, bool isDesktop) {
    return Container(
      ...
      child: IconButton(
        icon: Icon(Icons.arrow_back, color: isDesktop ? Colors.white : Colors.black87, size: fs(18)),
  ...
  Widget _buildContactInfoSection(double Function(double) fs, double Function(double) hp, bool isDesktop) {
    return Column(
      ...
      children: [
        Row(
          children: [
            Icon(Icons.contact_support, color: isDesktop ? Colors.white : const Color(0xFF804692), size: fs(20)),
            SizedBox(width: fs(8)),
            Text(
              "Get In Touch",
              style: TextStyle(
  ...
  Widget _buildContactMethods(double Function(double) fs, double Function(double) hp) {
    return Column(
      children: [
        _buildContactMethod(Icons.phone, "+1 (555) 123-4567", fs),
        SizedBox(height: hp(1)),
        _buildContactMethod(Icons.email, "support@churppy.com", fs),
        SizedBox(height: hp(1)),
  ...
  Widget _buildContactMethod(IconData icon, String text, double Function(double) fs) {
    return Row(
      children: [
        Icon(icon, size: fs(16), color: const Color(0xFF804692)),
        SizedBox(width: fs(8)),
        Text(text, style: TextStyle(fontSize: fs(13), color: Colors.grey[700])),
      ],
    );
  }
        ...
        child: Column(
          ...
          children: [
            if (!isTablet) ...[
              Row(
                children: [
                  Icon(Icons.message, color: const Color(0xFF804692), size: fs(20)),
                  SizedBox(width: fs(8)),
                  Text(
                    "Send us a Message",
                    style: TextStyle(
                ...
                child: SizedBox(
                  ...
                  child: FilledButton(
                    style: FilledButton.styleFrom(
                      ...
                      elevation: 2,
                      shadowColor: const Color(0xFF804692).withOpacity(0.3),
                    ),
