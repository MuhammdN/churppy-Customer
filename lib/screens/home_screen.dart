...
void initState() {
  ...
  // ✅ Always initialize before first build
  ...
  print("📍 initState called – waiting for location...");
}
...
Future<void> _getLocationAndLoad() async {
  try {
    ...
    if (!serviceEnabled) {
      print("⚠️ Location service is OFF. Asking user to enable it...");
    ...
    if (permission == LocationPermission.denied) {
      print("🟡 Asking user for location permission...");
    ...
    // fallback call if error
    // setState(() {
    //   futureBusinesses = BusinessService.fetchBusinesses(lat: 0, lng: 0);
    // });
  }
}
